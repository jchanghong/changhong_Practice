...
public class PhotoViewAttacher implements IPhotoView, View.OnTouchListener,
    ...
    private static float newrotation = 0;
    ...
    private double testthree(MotionEvent event){
        if (event.getPointerCount() == 3){
           double delta_y =(event.getY(0) + event.getY(1) + event.getY(2));
           return delta_y;
        }
        return 0;
    }
...
//     Override onTouch function to realize rotate with gestures.
    @Override
        ...
        if (mZoomEnabled && hasDrawable((ImageView) v)) {
           ...
           switch (ev.getAction() & MotionEvent.ACTION_MASK) {
                ...
                case MotionEvent.ACTION_POINTER_DOWN:
                    oldrotation =rotation(ev) +mLastRotation;
                    lastlocation =testthree(ev);
                    mode = 1;
                    break;
                case MotionEvent.ACTION_MOVE:

                    if (mode == 1) {
                        newrotation = rotation(ev) - oldrotation;
                        currentlocation +=testthree(ev)-lastlocation;
                        lastlocation =testthree(ev);
                        Log.i("YJY","r:"+newrotation+"    l:"+mLastRotation);
                        this.setPhotoViewRotation(-newrotation);
                    }
                    break;
                ...
                case ACTION_UP:
                    ...
                    // to min scale
//                    Log.i("yjy","current:"+(-currentlocation)+"height："+3*v.getHeight()/2);
                    if(-currentlocation > 3*v.getHeight()/2){
                        Toast toast = Toast.makeText(v.getContext(),"测试三点",Toast.LENGTH_SHORT);
                        toast.show();
